...
สร้างได้ 3 ชั้นตามผลลัพธ์:
  ...
  - "ตรวจสอบ: Node ไม่ตรงหมุด"     จุดสีแดง (โปร่งใส 40%)
...
class ResultLayerManager:
    ...
    def build(self, results, crs):
        ...
        :param results: list ของ dict {type, geometry, fids, area, detail, distance}
    ...
    def _create_layer(self, key, geom_type, items, crs):
        ...
        provider.addAttributes([
            ...
            QgsField("distance", QVariant.Double),  # ระยะถึงหมุดใกล้สุด (เฉพาะ node)
        ])
        layer.updateFields()
